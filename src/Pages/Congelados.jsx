...
const Congelados = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const data = await fetchProducts({ category: 'Congelados' });
        setProducts(data);
      } catch (error) {
        console.error('Error al obtener productos congelados:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <>
      ...
      <div className="container my-5">
        {loading && <p className="text-center">Cargando productos congelados...</p>}
          ...
          <div className="row g-4 product-grid justify-content-center">
            ...
            {filteredProducts.length === 0 && (
              <p className="text-center text-muted col-12">
                No se encontraron productos congelados con el término "{searchTerm}".
              </p>
            )}
          </div>
        )}
